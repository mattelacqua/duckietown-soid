...
def update(dt):
    ...
    if not agent0.actions:
        if agent0.intersection_detected(env):
            agent0.add_actions(agent0.handle_intersection(env, choice="Straight", forward_step=speed0))
        else: 
    ...
    if not agent1.actions:
        if agent1.intersection_detected(env):
            agent1.add_actions(agent1.handle_intersection(env, choice="Right", forward_step=speed1))
        else: 
